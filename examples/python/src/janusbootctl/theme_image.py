...
def render_preview(
    ...
    *,
    ...
    colors_raw = theme.get("colors")
    colors: dict[str, Any] = colors_raw if isinstance(colors_raw, dict) else {}
    bg_color = "#1A1A1A"
    ...
    draw.rectangle(
        [width // 10, height // 3, width * 9 // 10, height // 3 + 120], outline=accent, width=4
    )
    draw.text(
        (width // 10 + 24, height // 3 + 40),
        f"{name} preview ({width}x{height})",
        fill=text,
        font=font,
    )
