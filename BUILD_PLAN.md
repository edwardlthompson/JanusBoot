...
<!-- remaining-tally -->
**Remaining:** AGENT 84 · AUTO 0 · HUMAN 7 · ADB 0 · **91 open**
<!-- /remaining-tally -->

**Progress (not zero):** Sprint 0 ✅ done (archived) · Phase 0–2 ✅ **15/15** rows done · **12** Phase 3–8 stubs still open · **79** Template upgrade UPG rows boarded (1.6.0→1.8.0; apply not started). The “Remaining” line counts only 🔲/❌ — it does **not** mean nothing shipped. Archive: [`COMPLETED_TASKS.md`](COMPLETED_TASKS.md).
...
Next open work: **Template upgrade 1.6.0 → 1.8.0** (UPG-01…79 board rows) and **Phase 3–8** stubs.
...
Named gaps from `/upgrade` (parent `edwardlthompson/agent-project-bootstrap` **1.8.0**). Board rows only — **do not** mass-copy Sacred paths until `[HUMAN]` approves apply. Canon = copy; Mixed = merge + human review before ✅; Sacred = human approval required before any apply; Golden Path = optional product `/feature` (do not overwrite `janusbootctl`).
...
71. 🔲 [HUMAN] Sacred UPG-71: approve before any touch of `AGENTS.md` (never blind-overwrite)
72. 🔲 [HUMAN] Sacred UPG-72: approve before any touch of `examples/node/package-lock.json` (never blind-overwrite)
73. 🔲 [HUMAN] Sacred UPG-73: approve before any touch of `examples/node/package.json` (never blind-overwrite)
74. 🔲 [HUMAN] Sacred UPG-74: approve before any touch of `examples/python/pyproject.toml` (never blind-overwrite)
75. 🔲 [HUMAN] Sacred UPG-75: approve before any touch of `examples/python/uv.lock` (never blind-overwrite)
76. 🔲 [HUMAN] Sacred UPG-76: approve before any touch of `examples/web/package-lock.json` (never blind-overwrite)
77. 🔲 [HUMAN] Sacred UPG-77: approve before any touch of `examples/web/package.json` (never blind-overwrite)

#### Golden Path (78–79) — optional / product; adopt via `/feature` only

78. 🔲 [AGENT] Golden UPG-78 (optional): `/feature` `about` — About screen ([spec](docs/features/donations-updates.md)); do not overwrite janusbootctl with stub
79. 🔲 [AGENT] Golden UPG-79 (optional): `/feature` `crash-capture` — Crash capture ([spec](docs/features/crash-capture.md)); do not overwrite janusbootctl with stub
