...
- **Security Scanning:** GitHub Advanced Security (CodeQL), Dependabot, and Trivy container/dependency scanning.
  ...
  - **`[AGENT]`** Scaffold `.github/dependabot.yml` for each active package ecosystem (npm, pip, github-actions, gradle for Android, cargo for Rust, gomod for Go) with **weekly** grouped updates. Note: `dependabot.yml` schedules version-update PRs; **Dependabot alerts** are a separate GitHub repo setting for CVE advisories — both are required.
...
- **In-App About Screen & App Update Checker (product UI — not GitHub About, not template checker):**
  - **`[AGENT]`** Scaffold an accessible **About** screen reachable from Settings → App info only: app name, version, license/project URL, quiet **Donate via Venmo** (never in the header / titlebar). See `docs/help/DONATIONS.md` for GitHub Sponsors and international methods.
...
- **Weekly Security Triage:** GitHub Monday cron (`.github/workflows/weekly-health-check.yml`) already runs CI wait, Dependabot/Scorecard triage, and an `update-deps` dry-run. `[HUMAN]` only steps in if that workflow is red or a Critical/High alert needs a decision. Follow `docs/SECURITY_TRIAGE.md`. Log deferred Critical/High items in `DECISION_LOG.md`. Do not re-list weekly chores as 🔲 rows on `BUILD_PLAN.md`.
...
- **Incremental features (Sprint 2+):** One vertical-slice feature per BUILD_PLAN row. See `docs/FEATURE_MODULES.md`. After every `[AGENT]` step run `bash scripts/watch-agent-gates.sh --once --autofix`; agent may auto-fix lint/tests in feature scope until pass or 3-strike. After the sprint is all ✅, `python3 scripts/agent-run.py smoke-sprint --require` must pass before the next sprint. `git push` still requires `[HUMAN]` approval.
...
> **Canonical sign-off list:** `BUILD_PLAN_TEMPLATE.md` Sprint 0 AUTO row (`validate-bootstrap.sh --quick`, `feature-gate.sh --stack <active>`, `check-github-ci.sh --wait 300`, `check-license-compliance.sh`). Extended checks below are recommended before first push.
